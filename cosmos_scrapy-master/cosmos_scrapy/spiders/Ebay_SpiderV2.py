...
def insert_keywords(key_word, notes):
    client = MongoClient('mongodb://localhost:27017')
    page_dict = dict()
    page_dict['key_word'] = key_word
    page_dict['notes'] = notes
    client.ebay.key_word.insert_one(page_dict)  ##save the html url in the dic
    client.close()


def read_and_insert_to_db_csv():
    df_ebay = pd.read_csv('~/Desktop/0-0-jis files/key_words.csv')
    for index, row in df_ebay.iterrows():
        print(row['item_search'], row['notes'])
        keyword = row['item_search']
        note = row['notes']
        insert_keywords(keyword,note)
# read_and_insert_to_db_csv()
...
def save_html_source(key_word,html):
    ...
    page_dict['key_word'] = key_word
    page_dict['html'] = html
    page_dict['datetime'] = datetime.datetime.now()
